...
try:
    ...
    with open(config_path) as config_file:
        ...
        config = config['br_ri_calculation']
    ...
    blob_client_instance = blob_service_client_instance.get_blob_client(CONTAINERNAME1+'/BR_RAW_FILE', BLOBPCFILE, snapshot=None)
    ...
    blob_service_client_instance = BlobServiceClient(account_url=STORAGEACCOUNTURL, credential=STORAGEACCOUNTKEY)
    blob_client_instance = blob_service_client_instance.get_blob_client(CONTAINERNAME1+'/BR_RAW_FILE', BLOBCMPFILE, snapshot=None)
    ...
    os.rename(ab,"BR_CMPFile.csv")

    os.remove('BR_CMPFile.zip')
    ...
    filterData.to_excel("BR_filterFile.xlsx",index=False)

    df_1 = pd.read_excel("BR_filterFile.xlsx")
