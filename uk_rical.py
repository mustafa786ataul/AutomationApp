...
try:
    ...
    with open(config_path) as config_file:
        ...
        config = config['uk_ri_calculation']
    ...
    blob_client_instance = blob_service_client_instance.get_blob_client(CONTAINERNAME1+'/UK_RAW_FILE', BLOBPCFILE, snapshot=None)
    ...
    blob_service_client_instance = BlobServiceClient(account_url=STORAGEACCOUNTURL, credential=STORAGEACCOUNTKEY)
    blob_client_instance = blob_service_client_instance.get_blob_client(CONTAINERNAME1+'/UK_RAW_FILE', BLOBCMPFILE, snapshot=None)
    ...
    os.rename(ab,"UK_CMPFile.csv")

    os.remove('UK_CMPFile.zip')

    #Path Of PartnerCenter File
    PCworkfile = "UK_PartnerCenter.xlsx"
    CMPworkfile = "UK_CMPFile.csv"

    df = pd.read_excel(PCworkfile)
    filterData = df[df["ServiceName"]=="Virtual Machines"]
    filterData.to_excel("UK_filterFile.xlsx",index=False)

    df_1 = pd.read_excel("UK_filterFile.xlsx")
    ...
    final_data.to_excel('uk_abc.xlsx')

    #Repeat all Item Label 
    data_frame = pd.read_excel('uk_abc.xlsx')
    ...
    #PivotFile
    data_frame.to_excel('UK_PCPivot.xlsx',index=False)
    os.remove('UK_filterFile.xlsx')
    os.remove('uk_abc.xlsx')
    os.remove('UK_PartnerCenter.xlsx')
    log.info('CMP File Pivot Excel created with name UK_PCPivot.xlsx')
    ...
    writer = pd.ExcelWriter('UK_CMPPivot.xlsx')
    pt.to_excel(writer, sheet_name='Sheet1')
    writer.save()

    workfile1 = "UK_PCPivot.xlsx"
    workfile2 = "UK_CMPPivot.xlsx"
    ...
    Left_join.to_excel("UK_CompletedFile.xlsx",index=False)
    log.info('Final Calculated File created with name UK_CompletedFile.xlsx..')
    os.remove('UK_PCPivot.xlsx')
    os.remove('UK_CMPPivot.xlsx')
    os.remove('UK_CMPFile.csv')

    blob_service_client_instance = BlobServiceClient(account_url=STORAGEACCOUNTURL, credential=STORAGEACCOUNTKEY)
    blob_client_instance = blob_service_client_instance.get_blob_client(CONTAINERNAME2+'/UK_COMP_FILE', "UK_CompletedFile.xlsx", snapshot=None)

    print("\nUploading to Azure Storage as blob:\n\t" + "UK_CompletedFile.xlsx")

    # Upload the created file
    with open("UK_CompletedFile.xlsx", "rb") as data:
